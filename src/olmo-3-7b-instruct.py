...
WORDS = [
    "crane", "stone", "flint", "glade",
    "beach", "march", "plump", "slate"
]
...
print("• 🔵 : correct letter in the right place\n"
      "• ⚪ : wrong or not in word\n"
      "• 🟡 : correct letter in the wrong place (but exists elsewhere)")

while guesses_left > 0:
    guess = input(f"\nYour turn! Guess a 5-letter word (#{'*'*(guesses_left)}): ").lower().strip()
    ...
    print("".join([f"{temp_feedback[i]} " if temp_feedback[i] else '' for i in range(5)]))

    # Give hints to user
    correct = sum(1 for a, b in zip(feedback, SECRET_WORD) if a == '🔵')
    wrong_right_letter = sum(
        1 for a, b in zip(guess, SECRET_WORD)
        if b in guess and a != '🔵' 
    )
